...
function createParticle(id: number): Particle {
  ...
  let x = 0, y = 0;
  if (edge === 0) { x = randomBetween(0, 100); y = -5; }
  else if (edge === 1) { x = 105; y = randomBetween(0, 100); }
  else if (edge === 2) { x = randomBetween(0, 100); y = 105; }
  else { x = -5; y = randomBetween(0, 100); }
  ...
  return {
    ...
    shape: (["star", "circle", "diamond", "crescent"] as const)[Math.floor(Math.random() * 4)],
...
export default function GoalCelebration({ show, message, onClose }: GoalCelebrationProps) {
  ...
  useEffect(() => {
    if (show) {
      ...
      particlesRef.current = Array.from({ length: 120 }, (_, i) => createParticle(i));
    } else {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particlesRef.current = particlesRef.current.filter((p) => {
        if (p.life < p.delay) { p.life++; return true; }
        p.life++;
        ...
        const py = (p.y / 100) * canvas.height + p.vy * (p.life - p.delay) * 2 + 0.05 * (p.life - p.delay) ** 1.5;
  ...
  return (
    ...
    >
      ...
      >
        <div className="absolute -inset-12 rounded-full bg-teal-400/10 blur-2xl animate-pulse" />

        <div
          className="relative px-10 py-8 rounded-2xl text-center"
          style={{
            background: "linear-gradient(135deg, rgba(255,255,255,0.12) 0%, rgba(255,255,255,0.04) 100%)",
            boxShadow: "0 0 60px rgba(52,211,153,0.3), inset 0 1px 0 rgba(255,255,255,0.2)",
        ...
        >
          ...
          <div className="flex justify-center gap-2 mt-3">
            {["✦", "★", "✦"].map((s, i) => (
              <span
                key={i}
                className="text-teal-400 text-2xl animate-bounce"
          ...
          <p className="mt-4 text-white/50 text-sm transition-opacity duration-500" style={{ opacity: canDismiss ? 1 : 0 }}>
            {language === "ar" ? "اضغط في أي مكان للمتابعة" : "tap anywhere to continue"}
          </p>
        </div>
      </div>
    </div>
