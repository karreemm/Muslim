...
export default function ListenQuranPage() {
  ...
  return (
    <div className="bg-[#FFF5E4] text-teal-600 dark:bg-slate-900 dark:text-white min-h-screen w-full flex justify-center">
      <div className="w-[95%] max-w-[1500px] mx-auto mt-32 flex flex-col items-center gap-10">
        <h1 className="text-2xl md:text-4xl text-center">{t("listenQuran.title")}</h1>
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 bg-[#FFF5E4] text-black dark:bg-slate-900 dark:text-white p-5">
          {reciters.map((reciter) => (
            <Link
              key={reciter.id}
              href={`/listen-quran/reciter/${reciter.id}`}
              className="bg-white dark:bg-slate-800 px-5 py-3 border border-transparent shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 rounded-lg flex justify-between group hover:border-teal-600 hover:bg-opacity-70 dark:hover:bg-opacity-70 dark:hover:border-teal-500"
            >
