...
): Promise<{ numberEn: number; numberAr: string; arabic: string } | null> {
  try {
    if (typeof hadithNumber === "string") {
      hadithNumber = parseInt(hadithNumber, 10);
    }
    const response = await fetch(
      `https://api.hadith.gading.dev/books/${bookId}/${hadithNumber}`
    );
...
export async function getMultipleHadiths(
  bookId: string,
  startingNumber: number | string
): Promise<{ numberEn: number; numberAr: string; arabic: string }[]> {
  const hadiths = [];
