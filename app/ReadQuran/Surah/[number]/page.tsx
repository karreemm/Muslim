...
export default function SurahPage() {
  ...
  const hasNextSurah =
    currentSurahNumber && currentSurahNumber < surahNames.length;

  const SurahNameAr = surahNames.find(
    (s) => s.number.toString() === number
  )?.ar;
  const SurahNameEn = surahNames.find(
    (s) => s.number.toString() === number
  )?.en;
  ...
  useEffect(() => {
    ...
    if (
      surahNumber &&
      (parseInt(surahNumber) > 114 || parseInt(surahNumber) < 1)
    ) {
        ...
        <div className="relative mt-20 w-[90%] flex flex-col items-center">
                ...
                >
                  <div className="flex justify-center px-3 py-2 bg-gray-100 border-b border-gray-200 rounded-t-lg dark:border-gray-600 dark:bg-gray-700">
                    <h3 className="font-semibold text-gray-900 dark:text-white">
                      {surahNames[currentSurahNumber][
                        language as keyof (typeof surahNames)[0]
                      ].toString()}
                    </h3>
                  </div>
                </div>
            ...
            {hasPrevSurah && (
              <>
                ...
                >
                  <div className="flex justify-center px-3 py-2 bg-gray-100 border-b border-gray-200 rounded-t-lg dark:border-gray-600 dark:bg-gray-700">
                    <h3 className="font-semibold text-gray-900 dark:text-white">
                      {surahNames[currentSurahNumber - 2][
                        language as keyof (typeof surahNames)[0]
                      ].toString()}
                    </h3>
                  </div>
          ...
          <div className="flex flex-col items-center gap-5">
            <h1 className="md:text-5xl text-3xl font-bold text-teal-600 dark:text-white">
              {Surah[language]}{" "}
              {
                surahNames.find((s) => s.number.toString() === number)?.[
                  language as keyof (typeof surahNames)[0]
                ]
              }
            </h1>
          ...
          >
            ...
            <ShareButtons
              url={`https://muslim-one.vercel.app/ReadQuran/Surah/${currentSurahNumber}`}
            />
          </div>
        </div>
      </div>
